...
8. EM-0041 supersedes the original approval design: a trusted `workflow_run` on accepted `main` may
   emit the App-signed `independent-review` check only for an exact five-file promotion shape after
   accepted-base validation succeeds. The App has check-write plus read-only contents and pull
   requests; it cannot approve, write contents, merge, or deploy.
