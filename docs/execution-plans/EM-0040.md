...
8. A trusted `workflow_run` on accepted `main` may approve only that exact four-file promotion shape
   after its required check succeeds. Repository protection must require one approval after the last
   push; the workflow has pull-request review permission but no contents, merge, or deployment write.
