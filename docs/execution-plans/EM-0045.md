...
5. Exercise the existing gateway locally, then activate one exact accepted container only if an
   ...
   handling, disclosure, and TLS before changing public status copy.
...
- 2026-09-01: the existing container, REST, and Streamable HTTP MCP implementation passed its local
  protocol and deterministic checks. A no-spend Render preview is technically suitable for bounded
  testing but its free service sleeps after inactivity. No authenticated Render or DNS authority is
