...
| Attestation scope confusion | The trusted post-check workflow classifies an immutable merge-base diff between the exact PR base and validated head, with rename detection disabled so both sides remain visible. It rebinds the open PR's base and head immediately before signing. Ordinary PRs are an explicit no-op before App-token creation. Any docket-sensitive diff that is not exactly five newly added files for one receipt child fails closed. |
