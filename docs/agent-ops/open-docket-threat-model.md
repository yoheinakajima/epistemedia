...
| Attestation scope confusion | The trusted post-check workflow binds the canonical repository, default-branch ref, base SHA, and head SHA to the successful validation event. It classifies that immutable merge-base diff with rename detection disabled so both sides remain visible, then rebinds the open PR to the same repository/ref/base/head immediately before signing. Ordinary PRs are an explicit no-op before App-token creation. Any docket-sensitive diff that is not exactly five newly added files for one receipt child fails closed. |
...
It is specialized to exact open-docket receipt children. Ordinary pull requests can satisfy the
same protected check only through a separate App-authenticated exact-head review path; this
workflow neither signs nor reports a failure for an all-ordinary path set.
